<ul class="menu-sub">
  @if (isset($menu))
    @foreach ($menu as $submenu)
      ...
      @php
        $hasPermission = true; // Assume user has permission by default

        // Check user permissions against submenu permissions
        // Here you should replace 'permission_name' with the actual permission name associated with the submenu
        if (!auth()->user()->hasPermission($submenu->slug)) {
            $hasPermission = false; // User does not have permission
        }
      @endphp

      {{-- Show submenu only if user has permission --}}
      @if ($hasPermission)
        {{-- active menu method --}}
        @php
          $activeClass = null;
